...
@Config
@TeleOp(name = "Arm pidf Test", group = "Test")
public class ArmPidfTestOpMode extends LinearOpMode {
    ...
    public enum ControlMode {
        ...
        FULL_CONTROL("FULL"); // Full control
    ...
    ProfiledPIDController profiledPID2 = null; // For calculate with goal parameter
    // Control mode
    ...
    public static double gravityFFOutput = 0.0;    // motor power (-1.0 to 1.0)
    ...
    private AdjustableLengthArmFeedforward gravityFF;
    ...
    public void runOpMode() {
        ...
        gravityFF = new AdjustableLengthArmFeedforward(robot.Arm);
        ...
        while (opModeIsActive()) {
            ...
            if (targetChanged || profiledPID2 == null) {
                // Reset the PID controllers
                profiledPID2 = new ProfiledPIDController(kP, kI, kD, kF,
                        new WPILibMotionProfile.Constraints(maxVelocity, maxAcceleration));
                profiledPID2.reset(currentAngle); //Extremely important. Otherwise, it doesn't work when lowering
            }
            // Update previous button states
            ...
            gravityFFOutput = gravityFF.calculate(currentAngle, armAngularVelocity, armAngularAcceleration);
            double profiledOutput2 = profiledPID2.calculate(currentAngle, targetAngle);
...
// Determine final output based on control mode
            switch (controlMode) {
                case FF:
                    output = gravityFFOutput;
                    break;
                case PROFILED_WITH_GOAL_PID:
                    output = profiledOutput2;
                    break;
                case FULL_CONTROL:
                    output = profiledOutput2 + gravityFFOutput;
                    break;
                default:
                    output = gravityFFOutput;
            }
            ...
            packet.put("FF_gravity_Output", gravityFFOutput);
            packet.put("profiled_output_with_goal", profiledOutput2);
            ...
            telemetry.addData("FF gravity Output", gravityFFOutput);
            telemetry.addData("Profiled Output (with goal)", profiledOutput2);
    ...
    private void updateHardwareDescription() {
        ...
        if (!robot.Arm.isConfigEqual(config)){
            ...
            gravityFF.hardwareDescription = robot.Arm;

        }
        if (profiledPID2 != null) {
            profiledPID2.setPID(kP, kI, kD, kF);
        }
    }
